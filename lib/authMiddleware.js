...
async function loadUserProfile(userId) {
  try {
    ...
    const [roles, crmRoles] = await Promise.all([
      loadUserRoles(userId),
      loadCrmRoles(userId)
    ])
    data.roles = roles
    data.crmRoles = crmRoles
...
async function loadCrmRoles(userId) {
  try {
    const { data, error } = await adminSupabase
      .from('crm_user_roles')
      .select('role')
...
export function getUserRoles(user, profile) {
  ...
  if (profile?.crmRoles && Array.isArray(profile.crmRoles)) {
    profile.crmRoles.forEach(r => roles.add(r))
  }
...
export function isBoard(user, profile) {
  return hasAnyRole(user, profile, ['board', 'board_member'])
}

export function isPartner(user, profile) {
  return hasRole(user, profile, 'partner')
}
