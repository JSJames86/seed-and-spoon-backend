...
import { supabase as adminSupabase, getSupabaseForUser } from './supabaseClient'

/**
 * Verify authentication token and load user profile.
 * Returns the per-user Supabase client so callers can make RLS-scoped queries.
 * @param {string} authHeader - Authorization header value
 * @returns {Promise<{user, profile, error, client}>}
 */
export async function verifyAuth(authHeader) {
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return { user: null, profile: null, error: 'Missing or invalid authorization header', client: null }
  }

  try {
    const client = getSupabaseForUser({ headers: { authorization: authHeader } })

    const { data: { user }, error } = await client.auth.getUser()

    if (error || !user) {
      return { user: null, profile: null, error: 'Invalid or expired token', client: null }
    }

    // Load user profile from database
    const profile = await loadUserProfile(user.id)

    return { user, profile, error: null, client }
  } catch (error) {
    console.error('Auth verification error:', error)
    return { user: null, profile: null, error: 'Authentication failed', client: null }
  }
}
...
export function requireAdmin(handler) {
  return async (req, res) => {
    const { user, profile, error, client } = await verifyAuth(req.headers.authorization)
    ...
    // Attach user, profile, and user-scoped client to request
    req.user = user
    req.profile = profile
    req.supabase = client
...
export function optionalAuth(handler) {
  return async (req, res) => {
    const { user, profile, client } = await verifyAuth(req.headers.authorization)

    // Attach user, profile, and client if present, but don't block if not
    req.user = user || null
    req.profile = profile || null
    req.supabase = client || null
    return handler(req, res)
  }
}
